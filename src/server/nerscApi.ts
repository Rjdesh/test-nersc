...
import {
  ...
  DEFAULT_MACHINE_ID,
  DEFAULT_NERSC_USER_ID,
  GPU_MEMORY_METRIC,
  GPU_MEMORY_BANDWIDTH_VALUE_SCALE,
  GPU_UTILIZATION_METRIC,
  GPU_UTILIZATION_VALUE_SCALE,
  JOB_FETCH_CONCURRENCY,
  LDMS_API_URL,
  NERSC_COMPUTE_API_URL,
  NODE_POWER_VALUE_SCALE,
...
import type {
  FetchJobMetricRecordsOptions,
  IrisJobPerformanceSummary,
  JobMetricsSummary,
  LdmsTaskResponse,
  LdmsTaskResultResponse,
  MetricRecord,
  SfApiCredentials,
} from './types.js';
...
const getOriginalErrorMessage = (error: unknown, fallback: string) => {
...
const fetchAccessToken = async (
  ...
  if (!response.ok) {
    ...
    console.error(
      `[nersc] response POST OIDC token endpoint ${response.status} (${Date.now() - startedAt}ms): ${responseText}\n`
    );
...
const fetchLdmsJson = async <T>(
  ...
  console.info(
    `[nersc] request ${method} LDMS ${pathname}${init.body ? ` body=${String(init.body)}` : ''}\n`
  );
  ...
  console.info(
    `[nersc] response ${method} LDMS ${pathname} ${response.status} (${Date.now() - startedAt}ms): ${responseText}\n`
  );
...
const metricSeriesValues = (
  records: MetricRecord[],
  metricName: string,
  scale: number,
  fractionDigits = 1
) => {
  const groups = new Map<string, { order: number; timestampValue: number | null; values: number[] }>();
...
const fetchJobPowerMetricRecords = async (
  accessToken: string,
  jobId: number,
  userId: string,
  machineId: string,
...
const fetchJobMetricsRecords = async (
  accessToken: string,
  jobId: number,
  userId: string,
  machineId: string,
...
export const fetchIrisGpuUtilization = async (
  env: Record<string, string>
) => {
...
export const fetchJobGpuMemoryMetrics = async (
  env: Record<string, string>,
  jobId: number,
...
export const fetchJobMetricsSummary = async (
  ...
  const metricSummary = (
    metricKey: keyof typeof JOB_METRIC_DEFINITIONS,
    scale: number,
    fractionDigits = 1
  ) => (
    metricsResult.status === 'fulfilled'
      ? averageMetricValue(
        metricsResult.value,
        JOB_METRIC_DEFINITIONS[metricKey].apiMetricName,
        scale,
        fractionDigits
      )
      : { average: null, sampleCount: 0 }
  );
  const powerSummary = (result: PromiseSettledResult<MetricRecord[]>, metricKey: 'nodePower' | 'cpuPower') => (
    result.status === 'fulfilled'
  ...
  if (errors.length) {
    console.warn(
      `[api] Job metrics summary partial failure for ${jobId} (${machineId}/${userId}): ${errors.join(' ')}\n`
    );
  }

  const summary: JobMetricsSummary = {
    ...
    series: {
      gpuUtilization: metricsResult.status === 'fulfilled'
        ? metricSeriesValues(
          metricsResult.value,
          JOB_METRIC_DEFINITIONS.gpuUtilization.apiMetricName,
          GPU_UTILIZATION_VALUE_SCALE,
          3
        )
        : [],
      cpuPower: cpuPowerResult.status === 'fulfilled'
        ? metricSeriesValues(
          cpuPowerResult.value,
          JOB_METRIC_DEFINITIONS.cpuPower.apiMetricName,
          NODE_POWER_VALUE_SCALE
        )
        : [],
      nodePower: nodePowerResult.status === 'fulfilled'
        ? metricSeriesValues(
          nodePowerResult.value,
          JOB_METRIC_DEFINITIONS.nodePower.apiMetricName,
          NODE_POWER_VALUE_SCALE
        )
        : [],
    },
...
export const fetchJobMetadataTaskId = async (
  env: Record<string, string>,
  jobId: string,
  userId: string
) => {
...
export const fetchNerscJobDetails = async (
  env: Record<string, string>,
  jobId: string
) => {
  const credentials = getCredentialsFromEnv(env);
  const accessToken = await fetchAccessToken(credentials);
  const fetchJob = async (useSacct: boolean) => {
  ...
  if (response.status === 500) {
    const responseText = await response.text();
    console.warn(
      `[api] NERSC sacct job details failed for ${jobId}; retrying without sacct: ${response.status} ${responseText}\n`
    );
  ...
  if (!response.ok) {
    const responseText = await response.text();
    throw new NerscApiError(
      `NERSC job request failed for ${jobId}: ${response.status} ${responseText}`,
      response.status
    );
  }
