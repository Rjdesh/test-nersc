...
import { CACHE_TTL_MS, IRIS_GPU_UTILIZATION_API_PATH, JOB_GPU_MEMORY_API_PATH, JOB_METRICS_SUMMARY_API_PATH, JOB_METADATA_TASK_API_PATH, NERSC_JOB_DETAILS_API_PATH, } from './constants.js';
import { loadServerEnv } from './env.js';
import { fetchIrisGpuUtilization, fetchJobMetadataTaskId, fetchJobMetricsSummary, fetchJobGpuMemoryMetrics, fetchNerscJobDetails, NerscApiError, } from './nerscApi.js';
loadServerEnv();
...
const setContentTypeHeader = (res, filePath) => {
    const extension = path.extname(filePath).toLowerCase();
    res.setHeader('Content-Type', MIME_TYPES[extension] ?? 'application/octet-stream');
};
const isCacheableAssetPath = (pathname) => (CACHEABLE_ASSET_PATHS.some((assetPath) => pathname.startsWith(assetPath)));
...
const serveStaticFile = async (res, filePath, pathname) => {
    try {
...
const serveFrontend = async (req, res, requestUrl) => {
...
const handleIrisGpuUtilization = async (req, res, requestUrl) => {
    if (req.method !== 'GET') {
        sendJson(res, 405, { error: 'Method not allowed' });
        return;
    }
...
const handleJobGpuMemory = async (req, res, requestUrl) => {
    if (req.method !== 'GET') {
        sendJson(res, 405, { error: 'Method not allowed' });
        return;
    }
    ...
    try {
        const payload = await fetchJobGpuMemoryMetrics(env, jobId, requestUrl.searchParams.get('userId'), requestUrl.searchParams.get('machineId'));
        setSuccessHeaders(res);
        res.end(payload);
    }
    catch (error) {
...
const handleJobMetricsSummary = async (req, res, requestUrl) => {
    if (req.method !== 'GET') {
        sendJson(res, 405, { error: 'Method not allowed' });
        return;
    }
    ...
    console.info(`[api] request GET ${JOB_METRICS_SUMMARY_API_PATH} jobId=${jobId} userId=${requestUrl.searchParams.get('userId') ?? ''} machineId=${requestUrl.searchParams.get('machineId') ?? ''}\n`);
    try {
        const payload = await fetchJobMetricsSummary(env, jobId, requestUrl.searchParams.get('userId'), requestUrl.searchParams.get('machineId'));
        console.info(`[api] response GET ${JOB_METRICS_SUMMARY_API_PATH} 200: ${payload}\n`);
        setSuccessHeaders(res);
        res.end(payload);
    }
    catch (error) {
        ...
        console.error(`[api] response GET ${JOB_METRICS_SUMMARY_API_PATH} 500: ${JSON.stringify(payload)}\n`);
        sendJson(res, 500, payload);
    }
};
const handleJobMetadataTask = async (req, res) => {
    if (req.method !== 'POST') {
        sendJson(res, 405, { error: 'Method not allowed' });
        return;
    }
...
const handleNerscJobDetails = async (req, res, requestUrl) => {
    if (req.method !== 'GET') {
        sendJson(res, 405, { error: 'Method not allowed' });
        return;
    }
